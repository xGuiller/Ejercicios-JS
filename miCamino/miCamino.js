...
for (let index = 0; index < miCamino.material.length; index++) {
  const dato = miCamino.material[index];
  if(dato.nombre == "Piolets"){

    console.log(`Lo primero que debo llevar es ${dato.nombre} y es ${dato.descripcion}`)
  }
  else if(dato.nombre == "Crampones"){

    console.log(`Lo segundo que debo llevar es ${dato.nombre} y es ${dato.descripcion}`)
  }
  else if(dato.nombre == "Cuerdas"){

    console.log(`Lo tercero que debo llevar es ${dato.nombre} y es ${dato.descripcion}`)
  }
  else{

    console.log(`y por ultimo debo llevar la ${dato.nombre} y es ${dato.descripcion}`)
  }
...
for (let i = 0; i < miCamino.cimas.length; i++){
  const cumbre = miCamino.cimas[i];
  
  console.log(`para subir al ${cumbre.nombre} debo subir ${cumbre.altitud} metros y debo viajar a ${cumbre.localizacion}`)
